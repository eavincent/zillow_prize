...
p1<-ggplot(missingness,aes(x=reorder(var, percent),y=percent,fill=category)) +
  geom_col() +
  scale_fill_manual(breaks=c(">=99%","<99%"),values=c("gray70","gray30")) +
  ...
  scale_x_discrete(breaks=paste0(as.character(var_key$category),"_missing"), labels=as.character(var_key$label)) +
  coord_flip()
...
p2<-ggplot(cor_logerror_missing,aes(x=reorder(type, abs(cor_logerror_missing)), y=abs(cor_logerror_missing),fill=ifelse(cor_logerror_missing < 0, "Negative","Positive"))) +
  ...
  scale_y_continuous(limits = c(NA,0.100),expand = c(0,0)) +
  theme_bw() +
  theme(panel.grid.minor = element_blank(),panel.grid.major.y = element_blank(),legend.position = c(0.875,0.09), axis.ticks=element_blank(),plot.margin = unit(c(0.01,0.03,0.01,0), "null")) +
  coord_flip() +
