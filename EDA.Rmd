...
packages<-c("data.table","corrplot","dplyr","ggplot2","magrittr","stringi","stringr","rebus","gridExtra","reshape2","ggmap","tidyr","caret")
...
missingness<-as.data.frame(missingness)
missingness$var<-colnames(imputed_train)
missingness<-missingness[order(missingness$missingness,decreasing=T),]
missingness$percent<-round(missingness$missingness/nrow(train)*100,3)
row.names(missingness)<-seq(1:nrow(missingness))
...
pred_by_mean_MAE<-sum(abs(train$pred_logerror_mean - train$logerror))/nrow(train)
...
sample_submission$X201610<-round(mean(train$logerror),5)
sample_submission$X201611<-round(mean(train$logerror),5)
sample_submission$X201612<-round(mean(train$logerror),5)
...
train.glm<-glm(as.formula(paste0("logerror ~ ",paste(lm_vars, collapse = " + "))), family=gaussian, data=train)
...
# To try:
# Linear model that uses only missingness to estimate logerror
# Linear model that uses combination of numeric data and missingness
# Linear model that uses categorical data....somehow
# Move on to ML models
# Calculate missingness for each data point -- see if that correlates with logerror
  # Can do this directly from the df_missingness binary matrix
```
